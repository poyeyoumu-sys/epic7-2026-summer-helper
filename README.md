# 2026 夏活辅助 · Tauri + Rust + Mantine + MaaFramework

这是从原 PySide6/Python 工程整体迁移出来的新工程。原程序完整保留在 `legacy-python/`，新程序的界面、控制、识别、策略和运行循环均位于 Tauri/Rust 工程中。

## 已迁移内容

- PySide6 UI 改为 Tauri 2 + React + Mantine。
- Python runner 改为 Rust 后台线程。
- 普通 ADB `screencap -p` 截图后端。
- MaaFramework `EmulatorExtras` 高速截图后端。
- MAA 初始化失败时可自动回退到 ADB。
- 原 1920×1080 识别区域与 PNG 模板已全部迁移。
- 装备分优先动态规划。
- 200 米优先固定路线：`跑跑冲保保幸保冲幸幸`。
- LV0 至 LV5 幸运初始数量、上限和补充米数。
- 100m/200m 资源补充。
- 技能选中确认、弱确认、pending 延迟结算、助跑后退动画锁定。
- 200m 后停止识别米数，只处理奖励页与高置信 cutdown。
- 本地 JSON 设置记忆，重启 EXE 后保留上次设备、截图后端和策略。
